#!/usr/bin/env python3
"""
wifi_microros_besteffort_runner.py  —  WiFi micro-ROS RTT experiment runner
                                        (BEST_EFFORT QoS).
...
Prerequisites (in order):
    ...
    5. Run this script:
           python3 wifi_microros_besteffort_runner.py --freq 100 --run 1

Output:
    data/wifi_microros_besteffort_100hz_run01_<timestamp>.csv
"""
...
class WiFiMicroROSBestEffortRunner(Node):
    """
    ...
    def __init__(self, freq_hz: int, run_idx: int):
        super().__init__("wifi_microros_besteffort_runner")
        ...
        # Default rclpy QoS is RELIABLE, which will NOT match BEST_EFFORT over
        # DDS/UDP — you'll see "incompatible QoS" warnings and zero replies.
        qos = QoSProfile(
...
def main():
    parser = argparse.ArgumentParser(
        description="WiFi micro-ROS RTT runner (BEST_EFFORT QoS)"
    )
    parser.add_argument(
    ...
    rclpy.init()
    node = WiFiMicroROSBestEffortRunner(freq_hz=args.freq, run_idx=args.run)
