#!/usr/bin/env python3
"""
serial_microros_besteffort_runner.py  —  Serial micro-ROS RTT experiment
                                          runner (BEST_EFFORT QoS).
...
Prerequisites (in order):
    1. Flash pico_micro_ros_example.c firmware to Pico
       Firmware must use rclc_*_init_best_effort for both pub and sub.
    ...
    3. Wait for agent to print "Session established"
    4. Run this script:
           python3 serial_microros_besteffort_runner.py --freq 100 --run 1

Output:
    data/serial_microros_besteffort_100hz_run01_<timestamp>.csv
"""
...
DATA_DIR          = Path("./data")
CONFIG_NAME       = "serial_microros_besteffort"

# How long to wait for the agent/Pico to be ready before sending
WARMUP_S = 3.0


class SerialMicroROSBestEffortRunner(Node):
    """
    ...
    QoS is BEST_EFFORT on both pub and sub to match the Pico firmware.
    """

    def __init__(self, freq_hz: int, run_idx: int):
        super().__init__("serial_microros_besteffort_runner")

        self.freq_hz  = freq_hz
        self.run_idx  = run_idx
        self.period_s = 1.0 / freq_hz

        # Match the Pico firmware, which uses rclc_*_init_best_effort on both
        # the voltage_request subscriber and voltage_reading publisher.
        qos = QoSProfile(
            reliability=QoSReliabilityPolicy.BEST_EFFORT,
            depth=10,
        )

        # Publisher: sends timestamp to Pico via micro-ROS agent
        self.pub = self.create_publisher(UInt64, "voltage_request", qos)

        # Subscriber: receives voltage reply from Pico via micro-ROS agent
...
def main():
    parser = argparse.ArgumentParser(
        description="Serial micro-ROS RTT runner (BEST_EFFORT QoS)"
    )
    parser.add_argument(
    ...
    rclpy.init()
    node = SerialMicroROSBestEffortRunner(freq_hz=args.freq, run_idx=args.run)
    ...
    try:
        # Warmup — give agent/Pico time to fully establish session
